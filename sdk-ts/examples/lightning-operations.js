...
export class LightningOperations {
     ...
     */
    createLiftIntent(amountSats, assetId = 'gBTC') {
        return {
     ...
     */
    createLandIntent(amountSats, lightningInvoice, assetId = 'gBTC') {
        return {
     ...
     */
    async simulate31500ServiceRequest(action, assetId, amount) {
        const request = {
            action,
            assetId,
            amount,
     ...
     */
    async simulate31501ServiceResponse(request, invoice) {
        const response = {
            status: 'pending',
     ...
     */
    async executeLiftFlow(amountSats, assetId = 'gBTC') {
        console.log(`\n🚀 Starting gBTC Lift Flow: ${amountSats} sats`);
        try {
            // Step 1: 31500 Service Request
            const serviceRequest = await this.simulate31500ServiceRequest('lift_lightning', assetId, amountSats);
            // Step 2: Get Lightning invoice from gateway (simulated)
            const mockInvoice = `lnbc${amountSats}n1p3k8...`; // Mock invoice
            const serviceResponse = await this.simulate31501ServiceResponse(serviceRequest, mockInvoice);
     ...
     */
    async executeLandFlow(amountSats, lightningInvoice, assetId = 'gBTC') {
     ...
     */
    async monitorSession(sessionId, timeout = 120000 // 2 minutes
    ) {
...
export async function runLightningExamples() {
    ...
    try {
        ...
        // Execute land flow
        const landResult = await lightning.executeLandFlow(50000, 'lnbc500000n1p3k8...', 'gBTC');
