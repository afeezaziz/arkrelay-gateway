...
export class VtxoOperations {
     ...
     */
    createSplitIntent(vtxoId, splitAmounts, assetId = 'gBTC', feeAssetId = 'gBTC') {
        return {
     ...
     */
    createMultiVtxoIntent(assetId, totalAmount, recipientPubkey, sourceVtxos = [], feeAssetId = 'gBTC') {
        return {
     ...
     */
    createOptimalChangeIntent(assetId, amountNeeded, recipientPubkey, feeAssetId = 'gBTC') {
        return {
     ...
     */
    async simulateVtxoInventory(userPubkey, assetId) {
     ...
     */
    findOptimalVtxoCombination(availableVtxos, amountNeeded, strategy = 'optimal') {
     ...
     */
    calculateChangeAmount(selectedVtxos, amountNeeded, fees = 10) {
     ...
     */
    async executeSplitFlow(vtxoId, splitAmounts, assetId = 'gBTC') {
     ...
     */
    async executeMultiVtxoFlow(assetId, totalAmount, recipientPubkey) {
        ...
        try {
            ...
            const intent = this.createMultiVtxoIntent(assetId, totalAmount, recipientPubkey, selectedVtxoIds);
     ...
     */
    async executeOptimalChangeFlow(assetId, amountNeeded, recipientPubkey) {
...
export async function runVtxoExamples() {
    ...
    try {
        // Split VTXO
        const splitResult = await vtxoOps.executeSplitFlow('vtxo_large_001', [200000, 300000], 'gBTC');
        ...
        const changeResult = await vtxoOps.executeOptimalChangeFlow('gBTC', 123456, 'npub1recipient...');
